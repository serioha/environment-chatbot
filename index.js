...
function handleMessage(sender_psid, message) {
  ...
  const locationAttachment =
    message &&
    message.attachments &&
    message.attachments.find((a) => a.type === 'location')
  const coordinates =
    locationAttachment &&
    locationAttachment.payload &&
    locationAttachment.payload.coordinates

  if (coordinates && !isNaN(coordinates.lat) && !isNaN(coordinates.long)) {
    handleMessageWithLocationCoordinates(
      sender_psid,
      coordinates.lat,
      coordinates.long
    )
    ...
    message.nlp.entities.location &&
    message.nlp.entities.location.find((g) => g.confidence > 0.8 && g.suggested)
  ) {
    const locationName = message.nlp.entities.location.find(
      (loc) => loc.confidence > 0.8 && loc.suggested
    )
    ...
    message.nlp &&
    message.nlp.entities &&
    message.nlp.entities.greetings &&
    message.nlp.entities.greetings.find(
      (g) => g.confidence > 0.8 && g.value === 'true'
    )
  ) {
...
function handleConfirmLocation(
  sender_psid,
  geocoding_location,
  geocoding_formattedAddr
) {
  console.log('Geocoding api result: ', geocoding_location)
  const query = { $and: [{ user_id: sender_psid }, { status: AUSTRALIA_YES }] }
  const update = {
    $set: {
      'location.lat': geocoding_location.lat,
      'location.long': geocoding_location.lng,
      status: AU_LOC_PROVIDED,
    },
  }
...
function handleMessageWithLocationCoordinates(
  sender_psid,
  coordinates_lat,
  coordinates_long
) {
  ...
  const update = {
    $set: {
      'location.lat': coordinates_lat,
      'location.long': coordinates_long,
      status: AU_LOC_PROVIDED,
    },
  }
  const options = { upsert: false, new: true }
        ...
        elements: [
          {
            ...
            image_url:
              'http://www.wwf.org.au/Images/UserUploadedImages/416/img-bait-reef-coral-bleaching-rubble-1000px.jpg',
            buttons: [
              {
                type: 'postback',
          ...
          },
          {
            ...
            image_url:
              'http://www.wwf.org.au//Images/UserUploadedImages/416/img-planet-globe-on-moss-forest-1000px.jpg',
            buttons: [
              {
                type: 'postback',
          ...
          },
          {
            ...
            subtitle: 'Bio Survey',
            image_url:
              'http://www.wwf.org.au/Images/UserUploadedImages/416/img-koala-in-tree-1000px.jpg',
            buttons: [
              {
                type: 'postback',
                title: 'Go Bio Survey',
          ...
          },
          {
            ...
            subtitle: 'Canvassing',
            image_url:
              'http://www.wwf.org.au/Images/UserUploadedImages/416/img-hackathon-winners-2017-1000px.jpg',
            buttons: [
              {
                type: 'postback',
                title: 'Go Canvassing',
...
function handleStartSearchYesPostback(sender_psid) {
  const yesPayload = {
    text:
      ' Ok, I have to get to know you a little bit more for this. Do you live in Australia?',
    quick_replies: [
      {
        content_type: 'text',
        title: 'Yes!',
        ...
        elements: [
          {
            ...
            image_url:
              'http://awsassets.panda.org/img/original/wwf_infographic_tropical_deforestation.jpg',
...
function handlePreferencePostback(sender_psid, chatStatus) {
  ...
  if (
    chatStatus &&
    !isNaN(chatStatus.location.lat) &&
    !isNaN(chatStatus.location.long)
  ) {
    request(
      {
      ...
      (err, res, body) => {
        if (err) {
          ...
          let elements = bodyJson.data
            .filter((d) => {
              if (
                isNaN(d.location && d.location.latitude) ||
                isNaN(d.location && d.location.longitude)
              ) {
                return false
              }
              return (
                d.location.latitude < chatStatus.location.lat + 0.1 &&
                d.location.latitude > chatStatus.location.lat - 0.1 &&
                d.location.longitude < chatStatus.location.long + 0.1 &&
                d.location.longitude > chatStatus.location.long - 0.1
              )
            })
            .slice(0, 3)
      ...
      if (err) {
        ...
        if (bodyObj.status === 'OK') {
          if (
            bodyObj.results &&
            bodyObj.results[0] &&
            bodyObj.results[0].geometry &&
            bodyObj.results[0].geometry.location
          ) {
            callback(
              sender_psid,
              bodyObj.results[0].geometry.location,
              bodyObj.results[0].formatted_address
            )
          }
        } else {
